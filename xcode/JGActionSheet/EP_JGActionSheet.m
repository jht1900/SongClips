...
@interface EP_JGActionSheetLayer : CAShapeLayer

@end

@implementation EP_JGActionSheetLayer
...
@interface EP_JGActionSheetTriangle : UIView

- (void)setFrame:(CGRect)frame arrowDirection:(JGActionSheetArrowDirection)direction;

@end

@implementation EP_JGActionSheetTriangle
...
+ (Class)layerClass {
    return [EP_JGActionSheetLayer class];
}

@end

@interface EP_JGActionSheetView : UIView

@end

@implementation EP_JGActionSheetView
...
@interface EP_JGActionSheetSection ()
...
@implementation EP_JGActionSheetSection
...
- (void)setButtonStyle:(JGActionSheetButtonStyle)buttonStyle1 forButton:(UIButton *)button {
    UIColor *backgroundColor, *borderColor, *titleColor = nil;
    UIFont *font = nil;
	
	JGActionSheetButtonStyle buttonStyle = buttonStyle1 & JGActionSheetButtonStyleMask;
	...
	// !!@ EP Mod
	if (buttonStyle1&JGActionSheetButtonStyleOptionFlushLeft)
	{
...
- (UIButton *)makeButtonWithTitle:(NSString *)title style:(JGActionSheetButtonStyle)style {
	...
	// !!@ EP Mod
	if (style&JGActionSheetButtonStyleOptionFlushLeft)
	{
...
@interface EP_JGActionSheet () <UIGestureRecognizerDelegate> {
    UIScrollView *_scrollView;
    EP_JGActionSheetTriangle *_arrowView;
    EP_JGActionSheetView *_scrollViewHost;
    ...
    UIView      *_showInView;
    //CGRect      _showInRect;
    CGPoint     _showInPoint;
}

@end

@implementation EP_JGActionSheet
...
- (instancetype)initWithSections:(NSArray *)sections {
    ...
    if (self) {
        ...
        _scrollViewHost = [[EP_JGActionSheetView alloc] init];
        ...
        for (EP_JGActionSheetSection *section in self.sections) {
            section.tag = index;
...
- (void)showFromPoint:(CGPoint)point inView:(UIView *)view arrowDirection:(JGActionSheetArrowDirection)arrowDirection animated:(BOOL)animated {
    ...
    ATRACE2(@"JGActionSheet showFromRect addSubview animated=%d _targetView=%@ " , animated, _targetView);
...
// ----------------------------------------------------------------------------------------------------------
//- (void)showFromPoint:(CGPoint)point inView:(UIView *)view animated:(BOOL)animated
//{
//    _showInPoint = point;
//    _showInView = view;
//    UIView *parentView = [[UIApplication sharedApplication] keyWindow];
//    
//    CGPoint pnt = [parentView convertPoint:point fromView:_showInView];
//    
//    [self showFromPoint:pnt inView:parentView arrowDirection:JGActionSheetArrowDirectionBottom animated:animated];
//    
//}
...
- (void)showFromRect:(CGRect)rect forView:(UIView *)view inView:(UIView *)parentView animated:(BOOL)animated
{
    _showInView = view;
    
    _showInPoint = (CGPoint){CGRectGetMidX(rect), rect.origin.y + rect.size.height};

    CGPoint pnt = _showInPoint;
    if (_showInView) pnt = [parentView convertPoint:_showInPoint fromView:_showInView];
    
    ATRACE2(@"JGActionSheet showFromRect pnt=%@ _showInView.bounds=%@ rect=%@", NSStringFromCGPoint(pnt), NSStringFromCGRect(_showInView.bounds), NSStringFromCGRect(rect));
    ATRACE2(@"JGActionSheet showFromRect _showInView=%@ " , _showInView);
    ATRACE2(@"JGActionSheet showFromRect parentView=%@ " , parentView);
    
    [self showFromPoint:pnt inView:parentView arrowDirection:JGActionSheetArrowDirectionTop animated:animated];
}
...
- (void)showFromUpdate
{
    ATRACE(@"showFromUpdate _showInPoint=%@ _showInView.bounds=%@", NSStringFromCGPoint(_showInPoint), NSStringFromCGRect(_showInView.bounds));
    ...
    CGPoint pnt = _showInPoint;
    if (_showInView) pnt = [_targetView convertPoint:_showInPoint fromView:_showInView];
    
    [self moveToPoint:pnt arrowDirection:JGActionSheetArrowDirectionBottom animated:NO];
}
...
- (void)anchorSheetAtPoint:(CGPoint)point withArrowDirection:(JGActionSheetArrowDirection)arrowDirection availableFrame:(CGRect)frame {
    ...
    if (!_arrowView) {
        _arrowView = [[EP_JGActionSheetTriangle alloc] init];
    ...
    if (!CGRectContainsRect(_targetView.bounds, finalFrame) || !CGRectContainsRect(_targetView.bounds, arrowFrame)) {
		// !!@ EP mod:
        ATRACE(@"WARNING: Action sheet does not fit within view bounds! Select a different arrow direction or provide a different anchor point!");
        ATRACE(@"WARNING: _targetView.bounds=%@", NSStringFromCGRect(_targetView.bounds));
        ATRACE(@"WARNING: finalFrame=%@", NSStringFromCGRect(finalFrame));
        ATRACE(@"WARNING: arrowFrame=%@", NSStringFromCGRect(arrowFrame));
    }
